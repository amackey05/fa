...
function _framework_path() {
  static $path = FALSE;
  if (!$path) {
    $matches = drupal_system_listing('framework\.info$', 'themes', 'name', 0);
    if (!empty($matches['framework']->filename)) {
      $path = dirname($matches['framework']->filename);
    }
  }
  return $path;
}
