...
if (theme_get_setting('ukadmissions_rebuild_registry')) {
 ...
 */
function ukadmissions_theme(&$existing, $type, $theme, $path) {
  // When #341140 is fixed, replace _ukadmissions_path() with drupal_get_path().
  include_once $_SERVER['DOCUMENT_ROOT'] . _ukadmissions_path() . '/zen-internals/template.theme-registry.inc';
  return _ukadmissions_theme($existing, $type, $theme, $path);
}
 ...
 */
function ukadmissions_breadcrumb($breadcrumb) {
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('ukadmissions_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('ukadmissions_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('ukadmissions_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('ukadmissions_breadcrumb_title')) {
        if ($title = drupal_get_title()) {
          $trailing_separator = $breadcrumb_separator;
        }
      }
      elseif (theme_get_setting('ukadmissions_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function ukadmissions_links($links, $attributes = array('class' => 'links'), $heading = '') {
  global $language;
  $output = '';
 ...
 */
function ukadmissions_menu_item_link($link) {
 ...
 */
function ukadmissions_menu_local_tasks() {
  $output = '';
 ...
 *
 * @see ukadmissions_show_blocks_discovery()
 */
 /*
function ukadmissions_blocks($region, $show_blocks = NULL) {
  ...
  // If ukadmissions_blocks was called with a NULL region, its likely we were just
 ...
 *
 * @see ukadmissions_blocks()
 */
function ukadmissions_show_blocks_discovery(&$vars) {
  if ($vars['show_blocks'] == FALSE) {
    // Allow ukadmissions_blocks() to statically cache the $show_blocks variable. A TRUE
 ...
 */
function ukadmissions_preprocess(&$vars, $hook) {
 ...
 */
function ukadmissions_preprocess_page(&$vars, $hook) {
  // If the user is silly and enables Zen as the theme, add some styles.
  if ($GLOBALS['theme'] == 'zen') {
    include_once './' . _ukadmissions_path() . '/zen-internals/template.zen.inc';
    _ukadmissions_preprocess_page($vars, $hook);
  }
  ...
  if (theme_get_setting('ukadmissions_wireframes')) {
 ...
 */
function ukadmissions_preprocess_maintenance_page(&$vars, $hook) {
 ...
 */
function ukadmissions_preprocess_node(&$vars, $hook) {
 ...
 */
function ukadmissions_preprocess_comment(&$vars, $hook) {
  include_once './' . _ukadmissions_path() . '/zen-internals/template.comment.inc';
  _ukadmissions_preprocess_comment($vars, $hook);
}
 ...
 * @see region.tpl.php
 */
function ukadmissions_preprocess_region(&$vars, $hook) {
...
function ukadmissions_preprocess_block(&$vars, $hook) {
  $block = $vars['block'];
  ...
  if (theme_get_setting('ukadmissions_block_editing') && user_access('administer blocks')) {
    //include_once './' . _ukadmissions_path() . '/zen-internals/template.block-editing.inc';
    ukadmissions_preprocess_block_editing($vars, $hook);
 ...
 *   The name of the template being rendered.
 */
function ukadmissions_process(&$vars, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function ukadmissions_process_block(&$vars, $hook) {
 ...
 * replace _ukadmissions_path() with drupal_get_path('theme', 'zen').
 */
function _ukadmissions_path() {
	global $base_url;
  static $path = FALSE;
  if (!$path) {
    $matches = drupal_system_listing('ukadmissions\.info$', 'themes', 'name', 0);
    if (!empty($matches['ukadmissions']->filename)) {
      $path = dirname($matches['ukadmissions']->filename);
    }
  }
...
function phptemplate_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  ...
  $rawlink = strip_tags($link);
  
  if(strstr($class, 'expanded')) {
  	$link = strip_tags($link);
	$sub_menu = (!empty($menu) ? '<ul class="child">'. $menu .'</ul>' : $menu);
	  if(!strstr($extra_class, 'last')) {
		$link = str_replace($rawlink, '<div class="branch_link"><span>'.$rawlink.'</span></div>', $link);
	  }

  } else {
	$sub_menu = (!empty($menu) ? '<ul class="child">'. $menu .'</ul>' : $menu);
	  if(!strstr($extra_class, 'last')) {
		$link = str_replace($rawlink, '<div class="child_link">'.$rawlink.'</div>', $link);
	  }
...
function phptemplate_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  ...
  if(strstr($class, 'expanded')) {
	...
	$sub_menu = (!empty($menu) ? '<ul class="child">'. $menu .'</ul>' : $menu);
	  if(!strstr($extra_class, 'last')) {
		//$link = str_replace($rawlink, '<div class="branch_link"><span>'.$rawlink.'</span></div>', $link);
		//$link = str_replace($rawlink, '<div class="branch_link">'.$rawlink.'</div>', $link);
		//$class .= ' branch_link';
		$pos = strrpos($link, $rawlink);
		$link = substr_replace($link, '<div class="link_corner"></div><div class="branch_link">'.$rawlink.'</div>', $pos, strlen($rawlink));
	  }
	...
	$sub_menu = (!empty($menu) ? '<ul class="child">'. $menu .'</ul>' : $menu);
	  //if(!strstr($extra_class, 'last')) {
		...
		//$class .= ' child_link';
		$pos = strrpos($link, $rawlink);
		$link = substr_replace($link, '<div class="link_corner"></div><div class="child_link">'.$rawlink.'</div>', $pos, strlen($rawlink));		
	 // }
  ...
  if(!strstr($extra_class, 'last')) {
	return '<li class="'. $class . '">'. $link . $sub_menu ."</li>\n";
	//return '<li class="'. $class . '">'. $link . $sub_menu ."<span class=\"last\"></span></li>\n";
  } else {
	return '<li class="'. $class . '">'. $link . $sub_menu ."<span class=\"last\"></span></li>\n";
  }
  //return '<li>'. $link . $menu ."</li>\n";
...
function ukadmissions_preprocess_views_exposed_form(&$vars, $hook) {
	if($vars['form']['#id'] == 'views-exposed-form-recruiters-page-1') {
