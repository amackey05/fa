...
function framework_preprocess_block(&$vars, $hook) {
  ...
  include_once './' . drupal_get_path('theme', 'frameworkandroid') . '/template.block-editing.inc';
...
function _framework_path() {
  static $path = FALSE;
  if (!$path) {
    $matches = drupal_system_listing('frameworkandroid\.info$', 'themes', 'name', 0);
    if (!empty($matches['frameworkandroid']->filename)) {
      $path = dirname($matches['frameworkandroid']->filename);
    }
  }
  return $path;
}
